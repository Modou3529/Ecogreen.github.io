...
.hero-tagline{font-size:clamp(15px,2.2vw,19px);color:var(--green);margin-bottom:14px;animation:fadeUp 0.6s 0.2s ease both;font-weight:800;}
...
@media(max-width:640px){#navbar{padding:0 16px;}.nav-links{display:none;}.hero-content{padding:90px 20px 60px;}.hero-stats{gap:28px;}.how-section,.manifesto,.team-section{padding:60px 20px;}.app-wrap,.saved-wrap{padding:28px 16px;}}
...
// ========== FIXED API CALL - WORKING VERSION ==========
...
async function callAPI(msgs, sys){
  ...
  // Add the system prompt and conversation
  ...
  // USING CORRECT MODEL: gemini-1.5-flash (works with your key)
  ...
  if(!response.ok){
    ...
    if(response.status === 403 || response.status === 400){
      GEMINI_KEY = '';
      localStorage.removeItem('edubuild-gemini-key');
      throw new Error('Invalid API key. Please get a valid key from aistudio.google.com/apikey');
    }
...
<div id="key-overlay">
  <div class="key-box">
    ...
    <input class="key-input" id="key-input" type="password" placeholder="Paste your API key here..." />
    <div class="key-btns">
